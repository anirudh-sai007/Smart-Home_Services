...
public class Upload_workimages extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_upload_workimages);
        firestore = FirebaseFirestore.getInstance();
        storage = FirebaseStorage.getInstance();
        s1=getIntent().getStringExtra("Phone");
        s2=getIntent().getStringExtra("Field");
        reference = firestore.collection("Service Provider").document("Service").collection(s2).document(s1).collection("PICS").document("WORK_IMAGES").collection("PICTURES");
                ...
                storageReference.child("userData/").child(imagesList.get(i).getImageName()).putFile(imagesList.get(i).getImageURI()).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                        if (task.isSuccessful()){
                            storageReference.child("userData/").child(imagesList.get(finalI).getImageName()).getDownloadUrl().addOnCompleteListener(new OnCompleteListener<Uri>() {
                                ...
                                public void onComplete(@NonNull Task<Uri> task) {
                                    ...
                                    if (task.isSuccessful()){
                                        ...
                                        Toast.makeText(Upload_workimages.this, "Couldn't save "+imagesList.get(finalI).getImageName(), Toast.LENGTH_SHORT).show();
                                    }
                            ...
                            progressDialog.setMessage("Uploaded "+counter+"/"+imagesList.size());
                            counter++;
                            Toast.makeText(Upload_workimages.this, "Couldn't upload "+imagesList.get(finalI).getImageName(), Toast.LENGTH_SHORT).show();
                        }
                    }
                });
            }
    ...
    private void saveImageDataToFirestore(final ProgressDialog progressDialog) {
        ...
        reference.add(dataMap).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
            ...
            public void onSuccess(DocumentReference documentReference) {
                Intent it3=new Intent(Upload_workimages.this,Home.class);
                startActivity(it3);
                finish();
